...
def maximize(board_state: State, k: int, prune: bool, tree, alpha: int, beta: int):  # boardState is state object, k is number of levels
    ...
    for child in board_state.children:
        exploredNodes += 1
        n = tree.create_node(f'{child.node_type.name}:{0}', child.id, parent=board_state.id)
        _, utility = minimize(child, k - 1, prune, tree, alpha, beta)

        n.tag = f'{child.node_type.name}:{utility}'
...
def decide(board_state: State, k: int, prune: bool, color: str):
    ...
    node = None
    tree = Tree()
    root = tree.create_node(f'{board_state.node_type.name}:{0}', board_state.id)
    time1 = time.time()

    pruning = Pruning(-float('inf'), float('inf'))
    node, utility = maximize(board_state, k, prune, tree, pruning.alpha, pruning.beta)

    root.tag = f'{board_state.node_type.name}:{utility}'
    totalTime = time.time() - time1
    tree.show()
    print(f'Execution time = {totalTime}')
    print(f'Number of Explored Nodes = {exploredNodes}')
    print('------------------------------------------------------')
    exploredNodes = 0
    return node
